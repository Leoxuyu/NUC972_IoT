...
static INT _uartConfigureUART(PVOID pvParam)
{
    ...
    if ( (param->ucParity != NUC_PARITY_NONE) &&
         (param->ucParity != NUC_PARITY_EVEN) &&
         (param->ucParity != NUC_PARITY_ODD)  &&
         (param->ucParity != (NUC_PARITY_ODD | NUC_PARITY_STICK)) &&
         (param->ucParity != (NUC_PARITY_EVEN | NUC_PARITY_STICK)) )
        return UART_ERR_PARITY_INVALID;

    /* Check the supplied number of data bits */
    if ( (param->ucDataBits != NUC_DATA_BITS_5) &&
         (param->ucDataBits != NUC_DATA_BITS_6) &&
         (param->ucDataBits != NUC_DATA_BITS_7) &&
         (param->ucDataBits != NUC_DATA_BITS_8) )
        return UART_ERR_DATA_BITS_INVALID;

    /* Check the supplied number of stop bits */
    if ( (param->ucStopBits != NUC_STOP_BITS_1) &&
         (param->ucStopBits != NUC_STOP_BITS_2) )
